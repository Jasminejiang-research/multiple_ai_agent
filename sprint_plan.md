...
- [ ] 定义 `RunRecord` table。
- [ ] 定义 `NodeOutput` table。
- [ ] 定义 `AgentOutput` table。
- [ ] 定义 `ProposalOutput` table。
...
- [ ] 定义 `SupervisorPlan` schema。
- [ ] Supervisor 负责拆任务。
- [ ] Supervisor 负责决定调用哪些 Agent。
...
- [ ] 创建 `workflows/multi_agent_graph.py`。
- [ ] Graph 开始调用 Supervisor。
- [ ] Graph 调用 Research Agent。
- [ ] Graph 调用 Strategy Agent。
- [ ] Graph 调用 Finance Agent。
- [ ] Graph 调用 Writer Agent。
- [ ] Graph 调用 Critic Agent。
- [ ] Graph 调用 Revision Node。
- [ ] 保存每个 Agent 输出。
